/**
 * E2E Mercado Pago — token vía SDK (TEST) + API backend.
 * Uso: node scripts/e2e-mp-payment-browser.mjs [--force-expire]
 */
...
async function createCardTokenViaSdk() {
    ...
    const tokenPayload = await page.evaluate(async ({ publicKey, card }) => {
...
async function main() {
    ...
    if (!MP_PUBLIC_KEY?.startsWith("TEST-")) {
        throw new Error("Se requiere Public Key TEST para tarjetas sandbox.");
    }
    ...
    console.log("\n2) Token tarjeta vía SDK Mercado Pago …");
    const cardToken = await createCardTokenViaSdk();
    console.log("   ✓ id:", cardToken.id?.slice(0, 18) + "…");
    ...
    const success = payStatus === "APPROVED" && record?.status === "APPROVED";

    console.log("\n=== RESULTADO:", success ? "✅ PAGO APROBADO" : "❌ FALLO", "===");
    ...
    if (ctx.restoreSubId) {
        if (success) {
            ...
            console.log("↩ Suscripción anterior → EXPIRED");
        } else if (ctx.restoreEndDate) {
            await prisma.subscription.update({
                where: { subscriptionId: ctx.restoreSubId },
                data: { subscriptionEndDate: ctx.restoreEndDate },
            });
            console.log("↩ Fecha de suscripción restaurada");
        }
    }
