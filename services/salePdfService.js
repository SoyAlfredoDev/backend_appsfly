...
/**
 * Genera buffer PDF de comprobante de venta para adjuntar al correo.
 */
export function generateSalePdfBuffer({
    sale,
    business,
    items = [],
    payments = [],
    ...
    return new Promise((resolve, reject) => {
        ...
        const title = `${documentLabel.toUpperCase()}${sale?.saleNumber ? ` N° ${sale.saleNumber}` : ""}`;
        ...
        [
            ...
            doc.text(formatCurrency(amount), summaryX + 110, y, { width: 100, align: "right" });
            y += isTotal ? 18 : 14;
        });
        ...
        if (payments.length > 0) {
            ...
            payments.forEach((payment) => {
                doc.text(`${payment.methodLabel}: ${formatCurrency(payment.amount)}`, 48, y);
                y = doc.y + 4;
            });
        }
        ...
        doc.font("Helvetica").fontSize(8).fillColor("#6b7280")
            .text(
                business?.businessReceiptFooterNote?.trim()
                    || "Documento informativo generado por AppsFly.",
                48,
                footerY + 8,
